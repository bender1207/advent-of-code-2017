...
object Day08_1 extends App
{
  ...
  def executeInstruction(row: String, registerValueMap: mutable.Map[String, Int]) =
  {
  ...
  def getMaxRegisterValueAfterInstructions(rows: Seq[String]) : Int =
  {
    val registerValueMap: mutable.Map[String, Int] = collection.mutable.Map(rows.map(parseRegister).map((_, 0)): _*)
    rows.foreach(executeInstruction(_, registerValueMap))
    registerValueMap.maxBy(_._2)._2
  }
  ...
  // -- Tests -------------------
  {
    ...
    val max = getMaxRegisterValueAfterInstructions(rows)
    assert(max == 1)
  }
  // ----------------------------
